...
class GUI:
    def __init__(self):
        ...
        self.best_of = 3
        # Displaying scores for both players
        self.user_score_label = None
        self.computer_score_label = None
        # Displaying game text (outcome of each round)
        self.game_text = None
        # Setup Frames
        self.setup_frames()
    ...
    def setup_choice_frame(self) -> None:
        ...
        """
        choice_frame = tk.Frame(self.root, bd=10, relief="groove")
        choice_frame.grid(row=2, column=0, columnspan=3, pady=10)
        choice_text = tk.Label(choice_frame, text="Pick One:", font=(self.font, 30, "bold"), justify="center")
        choice_text.grid(row=0, column=0, columnspan=3)
        rock_button = tk.Button(choice_frame, text=self.choices[0], font=(self.font, 50),
                                command=lambda:self.update_score(self.choices[0]))
        rock_button.grid(row=1, column=0, padx=10, pady=2)
        paper_button = tk.Button(choice_frame, text=self.choices[1], font=(self.font, 50),
                                 command=lambda:self.update_score(self.choices[1]))
        paper_button.grid(row=1, column=1, padx=10, pady=2)
        scissors_button = tk.Button(choice_frame, text=self.choices[2], font=(self.font, 50),
    ...
    def update_score(self, user_choice) -> None:
        ...
        if user_choice == computer_choice:
            ...
            if user_choice == self.choices[0] and computer_choice == self.choices[2] or \
                ...
                playsound("assets/win.wav", block=False)
            else:
                self.game_text.config(text=f"You Lost", fg="red")
                self.computer_score += 1
                self.computer_score_label.config(text=f"Score: {self.computer_score}")
                playsound("assets/lose.wav", block=False)
        self.root.update_idletasks() # Prevent messagebox from popping up before score label updates
